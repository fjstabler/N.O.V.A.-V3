...
class SettingsStore:
    ...
    def patch(self, patch: dict[str, Any], *, persist: bool = True) -> NovaSettings:
        ...
        merged = _deep_merge(self._settings.model_dump(mode="json"), cleaned)
        try:
            updated = NovaSettings.model_validate(merged)
        except ValidationError as exc:
    ...
    def _write(self, settings: NovaSettings) -> None:
        try:
            ...
            document = tomlkit.dumps(settings.model_dump(mode="json"))
        except ImportError:  # pragma: no cover - tomlkit is a hard dependency
            document = _minimal_toml(settings.model_dump(mode="json"))
